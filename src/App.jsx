...
// BUILD0162 — okna vystředěna, maxHeight 90vh, bez zbytečného omezování
...
function useDraggable(w = 600, h = 500) {
  const calcPos = () => {
    ...
    return {
      ...
      y: Math.max(10, Math.round(iH / 2 - Math.min(h, iH * 0.95) / 2)),
    };
  };
...
function LogModal({ isDark, firmy, onClose, isDemo, isAdmin, isSuperAdmin }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1250, pointerEvents: "none", fontFamily: "'Segoe UI',Tahoma,sans-serif" }}>
      <div style={{ position: "fixed", left: pos.x, top: pos.y, pointerEvents: "all", background: modalBg, borderRadius: 16, width: "min(900px,97vw)", maxHeight: "90vh", display: "flex", flexDirection: "column", border: `1px solid ${isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.12)"}`, boxShadow: "0 32px 80px rgba(0,0,0,0.65)" }}>
